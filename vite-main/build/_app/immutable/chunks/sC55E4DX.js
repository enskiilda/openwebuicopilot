...
var _modified;
import { splitAccessPath, stringValue as $, mergeConfig, isObject, hasOwnProperty as has, isNumber, isString, logger, writeConfig, Warn, isArray, array, parseSelector as eventSelector, isBoolean as isBoolean$1, identity, parseExpression as parser, isFunction } from "./7VbVl9Il.js";
var version$1 = "6.4.1";
var pkg = {
  version: version$1
};
...
function hasProperty(obj, key) {
  return isObject(obj) && has(obj, key) && obj[key] !== void 0;
}
const ROW = "row";
const COLUMN = "column";
const FACET = "facet";
...
function isPolarPositionChannel(c) {
  return has(POLAR_POSITION_CHANNEL_INDEX, c);
}
...
function isGeoPositionChannel(c) {
  return has(GEO_POSIITON_CHANNEL_INDEX, c);
}
...
function isAggregateOp(a) {
  return isString(a) && has(AGGREGATE_OP_INDEX, a);
}
...
function selectionAsScaleDomainWithoutField(field) {
  return `A "field" or "encoding" must be specified when using a selection as a scale domain. Using "field": ${$(field)}.`;
}
function selectionAsScaleDomainWrongEncodings(encodings, encoding, extent, field) {
  return `${!encodings.length ? "No " : "Multiple "}matching ${$(encoding)} encoding found for selection ${$(extent.param)}. Using "field": ${$(field)}.`;
}
...
function isSortByChannel(c) {
  return has(SORT_BY_CHANNEL_INDEX, c);
}
...
function isStackOffset(s) {
  return has(STACK_OFFSET_INDEX, s);
}
...
function mergeDomains(domains) {
  ...
  const unionDomainSorts = unique(sorts.map((s) => {
    if (isBoolean(s) || !("op" in s) || isString(s.op) && has(MULTIDOMAIN_SORT_OP_INDEX, s.op)) {
      return s;
    }
  ...
  alignStackOrderWithColorDomain() {
    ...
    if (!order && Array.isArray(domain2) && typeof field === "string" && colorEncodingType === "nominal") {
      if (offsetEncoding && !offsetEncoding.sort) {
        ...
        const orderExpression = `indexof(${$(domain2)}, datum['${field}'])`;
